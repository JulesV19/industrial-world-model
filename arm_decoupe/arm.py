...
from config import (
    ...
    T_AMBIENT,
    TEMP_VISC_COEFF, TEMP_NOISE_COEFF, TEMP_DEADBAND_COEFF, TEMP_SENSOR_COEFF,
)
from degradation import friction_multiplier, noise_multiplier

class PhysicsArmEnv:
    def __init__(self, initial_q=None, machine_state=None, temperature: float = T_AMBIENT):
        """
        temperature  : temperature courante de la machine en degC.
                       Controle de facon deterministe les frottements, bruit moteur,
                       deadband et bruit capteur.
        machine_state: dict avec piece_count et cadence (usure mecanique).
        """
        ...
        # Usure mecanique (piece_count -> Hill saturation)
        fm = friction_multiplier(machine_state["piece_count"]) if machine_state else 1.0

        # Effet thermique deterministe (DeltaT = T - T_ambient)
        dT = max(0.0, temperature - T_AMBIENT)

        # Frottements : graisse chaude perd en efficacite -> visc augmente avec T
        self.visc     = np.array(visc_friction_coeffs) * fm + TEMP_VISC_COEFF * dT
        self.coulomb  = np.array(coulomb_friction_coeffs) * fm
        self.stribeck = np.array(stribeck_friction_coeffs) * fm

        # Bruit moteur : enroulements chauds -> resistance plus elevee -> plus de bruit
        self.motor_noise_std_eff = motor_noise_std * (1.0 + TEMP_NOISE_COEFF * dT)

        # Zone morte : dilatation thermique reduit legerement le jeu mecanique
        self.deadband_eff = deadband_torque + TEMP_DEADBAND_COEFF * dT

        # Capteurs : dilatation disque encodeur -> legere degradation precision angulaire
        self.sensor_noise_q_eff  = sensor_noise_q_std  + TEMP_SENSOR_COEFF * dT
        self.sensor_noise_dq_eff = sensor_noise_dq_std + TEMP_SENSOR_COEFF * dT * 5.0
    ...
    def read_sensors(self):
        ...
        # Bruit de mesure : residue gaussien x10 reduit, amplifie par vitesse et temperature
        q_noisy  = q_real  + np.random.normal(0, self.sensor_noise_q_eff  * vel_scale, 2)
        dq_noisy = dq_real + np.random.normal(0, self.sensor_noise_dq_eff * vel_scale, 2)
        # Quantification encodeur (deterministe)
        q_sensed = np.round(q_noisy / encoder_res) * encoder_res
        # Pas de glitches aleatoires : remplaces par degradation continue liee a T
