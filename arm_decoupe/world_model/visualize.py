...
from .dataset import (
    BINARY_IDX, COMPUTED_KEY_DIMS, METRIC_KEYS, Normalizer,
...
ALL_KEY_DIMS = {**dict(METRIC_KEYS), **COMPUTED_KEY_DIMS}
...
def _compute_loss_distribution(model, norm, H, full_ds, subset, device,
    ...
    for k in target_keys:
        if k in ("q_des", "q_real", "q_error"):
            break
        q_col += ALL_KEY_DIMS[k]
...
def _run_inference(model, norm, H, full_ds, subset, ep_idx, device,
    ...
    if "is_cutting" in target_keys:
        ...
        col = sum(ALL_KEY_DIMS[k] for k in target_keys[:ic])
    ...
    for k in target_keys:
        ...
        col += d
    if q_col is None:
        raise ValueError(f"Aucun signal de position (q_des/q_real/q_error) dans target_keys={target_keys}")

    predicts_error = target_keys == ["q_error"]
    if predicts_error:
        ep_paths = sorted(glob.glob(os.path.join(H["data_dir"], "episode_*.npz")))
        ep_path  = ep_paths[orig_idx]
        q_des_raw = np.load(ep_path)["q_des"].astype(np.float32)          # (T_full, 2)
        sf_loc    = max(1, int(H.get("subsample_factor", 1)))
        q_des_up  = _upsample(q_des_raw[::sf_loc, :2], sf_loc, T)         # (T, 2)
        q_fk_pred = q_des_up[:seq_len] + pred[:seq_len, q_col:q_col + 2]
        q_fk_true = q_des_up[:seq_len] + tgt [:seq_len, q_col:q_col + 2]
    else:
        q_fk_pred = pred[:seq_len, q_col:q_col + 2]
        q_fk_true = tgt [:seq_len, q_col:q_col + 2]
...
def browse(save_dir="world_model/checkpoints", split="val", step=1):
    ...
    def _load_ep(ep_idx):
        pred, tgt, waypoints, seq_len, q_col, target_keys, quality = _run_inference(
            ...
            dev_mean, dev_std)
        _, elbow_p, tip_p = fk(quality["q_fk_pred"])
        _, elbow_t, tip_t = fk(quality["q_fk_true"])
        orig_idx = subset.indices[ep_idx] if hasattr(subset, "indices") else ep_idx
        speed_val = float(full_ds.samples[orig_idx]["speed"])
        S.update(ep=ep_idx, fi=0, pred=pred, tgt=tgt,
                 waypoints=waypoints, seq_len=seq_len, target_keys=target_keys,
                 quality=quality, speed=speed_val,
    ...
    def _refresh_static():
        ...
        speed_text.set_text(f"Vitesse : {S['speed']:.3f} s/segment")
    ...
    def on_ep_change(val):
        ...
        title_text.set_text(
            ...
            f"({S['seq_len']} pas  ·  {S['speed']:.3f} s/seg)   "
