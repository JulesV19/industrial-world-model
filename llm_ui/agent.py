...
class SimulateCuttingInput(BaseModel):
    ...
    speeds: list[float] = Field(
        description=(
            ...
            "Valeur faible → rapide mais défauts. Plage valide : 1.0–3.0 s. Max 8 valeurs. "
            "Pour un balayage grossier utilise ~7 points espacés sur 1.0–3.0 s. "
            "Pour un affinage, utilise 6–8 points resserrés (±0.3 s) autour de la zone de transition."
        )
    )
...
class SimulateDrillingInput(BaseModel):
    ...
    speeds: list[float] = Field(
        description=(
            "Durées par segment à tester (secondes). Plage valide : 1.0–3.0 s. Max 8 valeurs. "
            "Pour un affinage, utilise des points resserrés (±0.3 s) autour de la zone de transition."
        )
    )
...
Temps alloué à chaque segment de trajectoire, **strictement dans la plage 1.0–3.0 s** (zone de validité du modèle) :
- Valeur faible (1.0–1.4 s) → production rapide ⚡, mais inertie et vibrations → défauts ❌
- Valeur élevée (2.2–3.0 s) → production lente 🐢, contrôleur stabilisé → qualité ✅
...
3. Explorer intelligemment la plage **1.0–3.0 s** en **deux passes** :
   - **Balayage grossier** : 7 points uniformes sur 1.0–3.0 s
   - **Affinage obligatoire** : dès que tu identifies la zone de transition (là où la qualité s'améliore nettement), relance une simulation avec 6–8 points resserrés dans un intervalle de ±0.3 s autour de cette zone, pour trouver le seuil précis
...
2. simulate_cutting(waypoints, [1.0, 1.3, 1.6, 1.9, 2.2, 2.6, 3.0])   ← balayage grossier
   → pour chaque vitesse : mean_deviation_mm, p95_deviation_mm, max_deviation_mm, real_corners

3. Identifier la zone de transition découpe (ex : entre 1.3 et 1.6 s)
   simulate_cutting(waypoints, [1.3, 1.38, 1.46, 1.54, 1.62, 1.7])     ← affinage obligatoire
   → trouver la vitesse minimale qui respecte le critère qualité

4. Récupérer les real_corners de la vitesse de découpe retenue

5. simulate_drilling(real_corners, [1.0, 1.3, 1.6, 1.9, 2.2, 2.6, 3.0]) ← balayage grossier
   → pour chaque vitesse : n_defects, mean_error_mm, per_corner_error_mm

6. Identifier la zone de transition perçage
   simulate_drilling(real_corners, [x.x, x.x, x.x, ...])                 ← affinage obligatoire

7. Recommander avec tableau récapitulatif et compromis chiffrés
```
