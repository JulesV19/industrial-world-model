"""
Visualise la distribution du taux de défauts en fonction de la vitesse d'exécution,
triée par forme (Square, Circle, Trapezoid).

Usage:
    python3 visualize_defect_vs_speed.py                    # données réelles uniquement
...
def load_model_records(data_dir: str, piece_db: list, model_path: str) -> list[dict]:
    import torch
    ...
    for ep_path in sorted(glob.glob(os.path.join(data_dir, "episode_*.npz"))):
        ...
        speed_val  = float(data["duration_per_segment"])
        T          = len(q_des)

        wps    = torch.tensor(np.array(piece_db[piece_idx], dtype=np.float32)).unsqueeze(0)
        wp_len = torch.tensor([wps.shape[1]])
        speed  = torch.tensor([[speed_val]], dtype=torch.float32)

        with torch.no_grad():
            q_pred_norm = model.predict(wps, wp_len, speed, max_len=T)  # (1, T, 2)

        q_pred = normalizer.denormalize(q_pred_norm[0].numpy())         # (T, 2)
        ...
        if n_cut == 0:
            ...
            deviation  = np.linalg.norm(fk_batch(q_pred) - fk_batch(q_des), axis=1)
            defect_pct = 100.0 * (deviation[cut_mask] > CUT_DEFECT_THRESHOLD).sum() / n_cut
